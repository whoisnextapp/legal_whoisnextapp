...
Bu politika; **WIN (Who is Next?)** platformu (“**Platform**”) üzerinde veya Platform’a bağlı altyapılarda tespit edilen **güvenlik açıklarının** ve **güvenlik zafiyetlerinin** nasıl bildirileceğini, bu bildirimlerin nasıl değerlendirileceğini ve giderileceğini, güvenlik araştırmacılarının hangi sınırlar içinde test yapabileceğini ve tarafların temel yükümlülüklerini açıklar.
...
<Callout type="warning" title="Önemli — Hızlı özet">
  ...
  bildirin: **`[SECURITY_CONTACT_EMAIL]`** (tercihen şifreli/PGP). - **Sosyal
  mühendislik**, **DDoS/DoS**, **kötü amaçlı yazılım**, **hesap ele geçirme** ve
  **kullanıcı verisine erişim** teşebbüsü yasaktır. - Testlerinizi **en az
  etki** ile ve yalnızca **gerekli asgari adımlarla** gerçekleştirin; Platform’u
  daha zayıf bir durumda bırakmayın. - İstemeden kişisel veriye veya özel
  içeriğe erişirseniz: **derhal durun**, veriyi çoğaltmayın/indirmeyin, bize
  bildirin ve talebimiz üzerine **güvenli şekilde silin**. - Bu politika;
  **KVKK**, **TCK** (özellikle özel hayat/haberleşme gizliliği) ve (uygulanırsa)
  **GDPR/DSA** çerçevesi ile uyumlu, “**iyi niyetli sorumlu açıklama**
  (responsible disclosure)” yaklaşımını benimser.
</Callout>
...
<Cards num={3}>
  ...
  >
    AB otoriteleri ve kullanıcı bildirim/itiraz mekanizmaları (uygulanırsa).
  </Card>
</Cards>
...
- **Web alanları**: `[WIN_PRIMARY_DOMAIN]` (örn. `whoisnextapp.com`) ve ilgili alt alan adları,
- **API uçları**: `[WIN_API_DOMAINS]` (örn. `api.whoisnextapp.com`, Firebase endpoint’leri vb.),
- **Dokümantasyon portalı**: bu portalın alan adı ve içerikleri,
- **Yetkilendirilmiş test ortamları** (varsa): `[WIN_STAGING_ENVIRONMENTS]`.

<details>
  <summary>**Kapsam içi varlık listesi (doldurulacak)**</summary>
  <ul>
    <li>
      **Web**: <code>[LIST_OF_WEB_DOMAINS_AND_SUBDOMAINS]</code>
    </li>
    <li>
      **API**: <code>[LIST_OF_API_DOMAINS]</code>
    </li>
    <li>
      **Mobil**: <code>[APP_STORE_GOOGLE_PLAY_LISTING_URLS]</code>
    </li>
    <li>
      **Diğer**: <code>[CDN_DOMAINS_PUSH_NOTIFICATION_DOMAINS]</code>
    </li>
  </ul>
</details>
...
- (Uygulanırsa) **GDPR** ve **DSA** (AB kullanıcıları/AB’ye yönelim ve şeffaflık/bildirim mekanizmaları bakımından).
...
### 3.1 Güvenlik ekibi iletişim noktası (taslak)

<Callout
  type="warning"
  title="Taslak (doldurulacak) — Güvenlik bildirim kanalları"
>
  - **E‑posta (tercih edilen)**: **`[SECURITY_CONTACT_EMAIL]`** - **PGP /
  şifreleme anahtarı (varsa)**: `[SECURITY_PGP_KEY_URL_OR_FINGERPRINT]` - **Web
  formu (varsa)**: `[SECURITY_REPORT_FORM_URL]` - **Bug bounty platformu
  (varsa)**: `[BUG_BOUNTY_PLATFORM_URL]`
</Callout>
...
>
  <Tab>
    **Teknik güvenlik açıkları** (yetkisiz erişim, veri ifşası, auth bypass vb.)
    için bu politika kapsamında **`[SECURITY_CONTACT_EMAIL]`** kanalını
    kullanın.
  </Tab>
  <Tab>
    Hesap erişimi/şifre unutma gibi destek konuları için:
    **`[SUPPORT_URL_OR_EMAIL]`**
  </Tab>
  <Tab>
    Taciz/tehdit, içerik ihlali veya kullanıcı güvenliği şikayetleri için
    uygulama içi “Bildir/Şikayet Et” akışını ve [Topluluk
    Kuralları](/topluluk-ve-guvenlik/topluluk-kurallari) sayfasını kullanın.
  </Tab>
  <Tab>
...
- Hesap erişimi/şifre unutma: `[SUPPORT_URL_OR_EMAIL]`
...
### 6.2 Hedef yanıt süreleri (taslak)

<Callout type="warning" title="Taslak (doldurulacak) — hedef süreler">
  - **Alındı onayı**: `[ACK_TARGET_BUSINESS_DAYS]` iş günü - **İlk teknik
  değerlendirme**: `[TRIAGE_TARGET_BUSINESS_DAYS]` iş günü - **Durum
  güncellemesi**: en az `[STATUS_UPDATE_FREQUENCY_DAYS]` günde bir (devam eden
  kritik konularda daha sık)
</Callout>
...
| Seviye     | Örnekler                                                                                           | Hedef giderim yaklaşımı (taslak)                                 |
| ---------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| **Kritik** | Toplu veri ifşası, kimlik doğrulama bypass, uzaktan kod çalıştırma, sistematik hesap ele geçirme   | `[CRITICAL_FIX_TARGET_DAYS]` gün içinde acil düzeltme/mitigasyon |
| **Yüksek** | Tekil kullanıcı verisi ifşası (yüksek etki), yetki yükseltme, ödeme/abonelikte kritik mantık açığı | `[HIGH_FIX_TARGET_DAYS]` gün                                     |
| **Orta**   | Sınırlı kapsamlı veri ifşası, belirli koşullarda istismar edilebilir zafiyet                       | `[MEDIUM_FIX_TARGET_DAYS]` gün                                   |
| **Düşük**  | Düşük etkili hatalar, savunma derinliği iyileştirmeleri                                            | `[LOW_FIX_TARGET_DAYS]` gün / planlı iyileştirme                 |
...
WIN’in **bug bounty (ödül) programı**:

- **Yoksa**: Bu politika bir “ödül taahhüdü” içermez. WIN, uygun görürse teşekkür/recognition sağlayabilir.
- **Varsa**: Ödül kriterleri, kapsam listesi ve eşik/severity koşulları `[BUG_BOUNTY_PLATFORM_URL]` üzerinden yayımlanır.
...
- bulgunun Platform’a etkisini (varsa) bize bildirin,
- üçüncü tarafın kendi güvenlik bildirim kanalına (varsa) da iletilmesi gerekebilir,
...
Güvenlik açığı bildirimi için: **`[SECURITY_CONTACT_EMAIL]`**  
Genel destek için: `[SUPPORT_URL_OR_EMAIL]`
