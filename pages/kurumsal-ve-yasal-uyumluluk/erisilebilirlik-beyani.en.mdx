...
- **Camera permission** and verification flow (selfie/liveness),
...
### 5.1 Registration/Login (Google, Apple and phone login)
...
### 5.5 Verification layer (camera permission, selfie/liveness)

In WIN, there may be verification steps such as camera permission and selfie/liveness for the purpose of "fake account prevention."
...
- Presenting key actions (pick, pass, profile details, block/report) with accessible labels,
...
WIN aims to provide an accessible experience for all users. The **known accessibility limitations** in the current version of the app are listed below; we will update this list as we resolve them:

- **Screen readers (VoiceOver / TalkBack):** Support is **partial**. Some elements such as the bottom menu tabs have accessible labels; however, some buttons, images and in particular the deck (card) flow may not be fully usable with a screen reader.
- **Larger text (system text size):** The system text size is reflected on many screens of the app; however, on deck cards the text scale is **capped at 130%** and in some fixed-size areas text may be shown scaled down to fit. Very large text settings are not fully supported.
- **Colour contrast:** White text on brand-gradient buttons and some small text on the chat screen (e.g. message times) do not meet the **WCAG 2.2 AA** contrast ratio everywhere. For this reason we do not declare the "Sufficient Contrast" feature in our store accessibility declarations.
- **Theme:** The app is offered only with a dark theme; there is no light theme option.
- **Reduce motion:** Page transitions and deck animations follow the device's "reduce motion" setting; some small animations (e.g. the loading logo, some feedback animations) may not be affected by this setting.
- **Voice messages:** No transcript or captions are provided for voice messages.
- **Selfie verification:** As stated in Section 5.5, there is currently no alternative technological verification method for users who cannot complete this step.
