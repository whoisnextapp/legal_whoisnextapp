...
Bu sayfa; WIN’in (Who is Next?) (“**Platform**”) **şeffaflık** ve **hesap verebilirlik** yaklaşımı kapsamında, periyodik olarak yayımlanacak **Şeffaflık Raporları**nın kapsamını, rapor türlerini, metriklerini ve metodolojisini açıklar.
...
<Cards num={2}>
  ...
  <Card title="Güvenlik İpuçları" href="/topluluk-ve-guvenlik/guvenlik-ipuclari" icon="🛡️">
  ...
  <Card title="Fikri Mülkiyet Politikası" href="/kurumsal-ve-yasal-uyumluluk/fikri-mulkiyet-politikasi" icon="🧩">
    Telif/marka ihlali bildirimleri ve uyar‑kaldır (takedown) prosedürü.
  </Card>
  <Card title="Güvenlik Açığı Bildirim Politikası" href="/kurumsal-ve-yasal-uyumluluk/guvenlik-acigi-bildirim-politikasi" icon="🧯">
...
<Callout type="info" title="İlk rapor dönemi">
  **İlk rapor dönemi (örn. 2026 Q3):** Uygulama henüz canlıya alınmadığı için bu dönemde raporlanacak veri bulunmamaktadır. Veri birikmeye başladığında, aşağıdaki şablona göre ilk gerçek rapor bu sayfaya eklenecektir.
</Callout>
...
- **Emirler**: (Uygulanabilir ise) AB’de DSA kapsamındaki “bilgi sağlama emri” ve “yasa dışı içeriğe karşı işlem emri” gibi araçlar.
...
### 3.1 Periyot (önerilen)

- **Yıllık**: En az yılda 1 kez (temel şeffaflık raporu paketi).
- **Ara dönem güncellemeleri (opsiyonel)**: Üç aylık/altı aylık özetler (özellikle hızlı büyüme veya yüksek riskli dönemlerde).
- **Olay bazlı özel rapor (opsiyonel)**: Büyük güvenlik olayı, sistemsel risk veya regülasyon gerektiren durumlarda.

### 3.2 Format (önerilen)

Raporlar mümkün olduğunca:

- **Web sayfası** (okunabilir ve erişilebilir),
- **İndirilebilir çıktı** (PDF) (opsiyonel),
- **Makine okunur özet** (CSV/JSON) (opsiyonel; kişisel veri içermeyecek şekilde)

formatlarında yayımlanır.
...
| **Fikri mülkiyet bildirimleri** (opsiyonel) | Telif/marka takedown görünümü | Yıllık | `Fikri Mülkiyet Politikası` |
| **Ürün güvenliği** (opsiyonel) | Güvenlik açığı bildirimleri ve giderim | Yıllık | `Güvenlik Açığı Bildirim Politikası` |
...
### 4.2 Temel metrik seti (önerilen)

| Metrik | Açıklama | Kırılımlar (örnek) |
|---|---|---|
...
### 4.3 Aksiyon türleri (örnek taksonomi)
...
- Otomasyon kullanımı (varsa) ve insan incelemesiyle ilişkisi.
...
- (Uygulanırsa) Art. 24/5 kapsamındaki kararların Komisyon veri tabanına iletimi hakkında özet.
...
### 5.5 AB ortalama aylık aktif alıcı (Art. 24/2)

AB hedefi varsa; geçmiş 6 ayın ortalaması olarak AB içindeki **ortalama aylık aktif alıcı** sayısı yayımlanır (kişisel veri içermez).
...
### 6.1 Talep türleri (önerilen sınıflandırma)

- **Bilgi/Belge talepleri** (hesap/profil, teknik log, içerik verisi (varsa), ödeme/abonelik).
...
- **İçerik/hesap emirleri**:
  ...
  - AB’de (uygulanırsa) DSA kapsamındaki emirler (bilgi sağlama / yasa dışı içeriğe karşı işlem).
...
| Emirler (opsiyonel) | Kaldırma/işlem emirleri | dayanak türü, ülke |
...
## Bölüm 7: Rapor Paketi D — Fikri mülkiyet bildirimleri (opsiyonel)
...
- karşı bildirim/itiraz sayıları (varsa),
...
## Bölüm 8: Rapor Paketi E — Ürün güvenliği (opsiyonel)
...
- Güvenlik açığı bildirim kanalı (opsiyonel).
...
- Topluluk güvenliği/bildirim süreçleri için: uygulama içi **Şikayet Et / Bildir** akışı (bkz. [Topluluk Kuralları](/topluluk-ve-guvenlik/topluluk-kurallari)).
