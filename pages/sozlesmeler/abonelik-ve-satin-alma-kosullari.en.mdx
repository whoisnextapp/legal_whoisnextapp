...
In addition to the WIN Premium subscription, one-time (consumable) purchases are offered through the stores: **deck packs** (1 / 2 / 4 decks) and **Visibility Week**. These products are not subscriptions and do not renew automatically; the conditions shown on the purchase screen and this text apply. For the use of purchased deck packs, see Section 3.7.

### 3.6 Virtual items

Initially there are **no** virtual items such as “coin/diamond/credit/gift”. If virtual items are offered later, the provisions in Section 10 apply. The deck packs in Section 3.5 and the free deck allowance in Section 3.7 are usage rights tied to the account; they cannot be transferred to another account or exchanged for cash.

### 3.7 Free allowance and the subscription requirement for choosing

Viewing profiles, being chosen, receiving notifications and messaging are provided free of charge. Choosing (finishing a deck and making a pick) is a paid feature. A "deck" is a set of 4 profiles shown two at a time; finishing a deck results in one pick.
...
Once the free allowance has been used, an active WIN Premium subscription is required to keep choosing. Deck packs bought as one-time purchases remain valid and can be used without a subscription until they are used up. Bonus decks granted without payment (for example through achievements or invitations) are promotional, have no monetary value, and can be used only while a WIN Premium subscription is active.
...
<Callout type="info" title="Summary of this section">
  One-time purchases (deck packs, Visibility Week) are governed by Section 3.5; this section sets out the general framework for any virtual items that may be added later.
</Callout>
