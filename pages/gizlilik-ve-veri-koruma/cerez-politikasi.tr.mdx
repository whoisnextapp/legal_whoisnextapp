...
Bu politika kapsamındaki çerez/izleme faaliyetleri bakımından veri sorumlusu, WIN Hizmetlerinin işletmecisidir: **WIN TECH Bilişim Organizasyon ve Ticaret A.Ş.** Veri sorumlusu iletişim bilgileri ve KVKK kapsamındaki başvuru yöntemleri için [KVKK Aydınlatma Metni](/gizlilik-ve-veri-koruma/kvkk-aydinlatma-metni) incelenmelidir. Her türlü soru için **support@whoisnextapp.com** adresi üzerinden bizimle iletişime geçebilirsiniz.
...
<Tabs items={["Web (Çerez)", "Mobil uygulama (İzleme/SDK)"]}>
  ...
  <Tab>
    ...
    - **Uygulama kurulum kimliği**: WIN’in tek cihaz oturumu ve hesap güvenliği için kullandığı kimlik (Android’de uygulamanın ürettiği “wn-” ile başlayan rastgele kimlik; iOS’ta IDFV),
...
- **Reklam gösterimi:** Uygulamada Google Mobile Ads (AdMob) SDK’sı yüklü olmakla birlikte **şu an uygulamada reklam gösterilmemektedir**. İleride reklam gösterilmesi hâlinde bu politika güncellenir ve gerekli izinler (ör. iOS’ta izleme izni, gerektiğinde açık rıza) ayrıca istenir.
- **Reklam kampanyası ölçümü (atıf):** WIN’in kendi reklam kampanyalarının etkisini ölçmek için uygulamada **Meta (Facebook) SDK** ve **TikTok Events SDK** kullanılır; satın alma ve yenileme olayları ayrıca **RevenueCat** üzerinden bu platformlara iletilebilir. Bu kapsamda aktarılan olaylar ve tanımlayıcılar [Gizlilik Politikası](/gizlilik-ve-veri-koruma/gizlilik-politikasi) Bölüm 4.14’te açıklanır. iOS’ta reklam tanımlayıcısı (IDFA) yalnızca izleme (ATT) izni verirseniz paylaşılır.
...
- **Google Mobile Ads / AdMob (Google):** SDK uygulamada yüklüdür; **şu an reklam gösterilmemektedir**.
- **RevenueCat (ABD):** Abonelik ve satın alma doğrulama/yönetimi; reklam ölçümü için cihaz tanımlayıcılarının Meta ve TikTok’a iletilmesi.
- **Meta (Facebook) SDK:** Uygulama olayları ve reklam kampanyası ölçümü (atıf). Facebook ile giriş seçeneği kaldırılmıştır.
- **TikTok Events SDK:** Uygulama olayları ve reklam kampanyası ölçümü (atıf).
- **Firebase Crashlytics (Google):** Uygulama çökme ve hata raporları.
...
Uygulamada şu an ayrı bir “izleme tercihleri” ekranı bulunmamaktadır. iOS’ta reklam tanımlayıcısının paylaşılmasını **Uygulama Takibi Şeffaflığı (ATT)** izniyle yönetebilirsiniz (Ayarlar > Gizlilik ve Güvenlik > İzleme). Android’de reklam kimliğinizi cihaz ayarlarından sıfırlayabilir veya silebilirsiniz. Pazarlama iletişimi ve yurt dışına aktarım rızalarınızı geri almak için [KVKK Başvuru Formu](/basvuru-formlari/kvkk-basvuru-formu) kanallarını kullanabilirsiniz.
...
<Tabs items={["Tarayıcılar", "iOS", "Android", "Ek opt-out araçları"]}>
  ...
  <Tab>
    ...
    - Uygulamada şu an reklam gösterilmemektedir; reklam ölçümüne ilişkin izinler bu bölümde anlatılan cihaz ayarlarıyla yönetilir.
...
Çerez/izleme teknolojileri kapsamında kişisel verilerinizin işlenmesine ilişkin haklarınız (örn. bilgi talebi, düzeltme, silme, işleme itiraz vb.), ilgili mevzuat kapsamında kullanılabilir. Başvuru yöntemleri ve veri sorumlusu iletişim bilgileri için [KVKK Aydınlatma Metni](/gizlilik-ve-veri-koruma/kvkk-aydinlatma-metni) incelenmelidir. Sorularınız için **support@whoisnextapp.com** adresine yazabilirsiniz.
...
| **IDFA / GAID (reklam tanımlayıcıları)** | Apple/Google | Pazarlama (ölçüm) | Meta, TikTok ve RevenueCat üzerinden reklam kampanyası ölçümü (atıf); iOS’ta yalnızca ATT izniyle. Şu an reklam gösterilmez | Kalıcı (Sıfırlanabilir) | Yurt Dışı |
| **Kurulum kimliği (wn-… / IDFV)** | WIN | Zorunlu | Tek cihaz oturumu; SMS doğrulama kötüye kullanım sınırları | Cihazda: uygulama kaldırılana kadar; sunucuda: hesap süresince | Google Cloud / Firebase |
| **RevenueCat SDK**          | RevenueCat    | Zorunlu (satın alma) | Abonelik/satın alma doğrulama ve yönetimi; atıf için cihaz tanımlayıcıları (IDFV, izin varsa IDFA, GAID, Android cihaz kimliği) ve IP adresinin iletilmesi | Hizmet süresince | Yurt Dışı (ABD) |
| **Meta (Facebook) SDK**     | Meta          | Pazarlama (ölçüm)  | Uygulama olayları ve reklam atfı                 | Sağlayıcı politikasına göre | Yurt Dışı |
| **TikTok Events SDK**       | TikTok        | Pazarlama (ölçüm)  | Uygulama olayları ve reklam atfı                 | Sağlayıcı politikasına göre | Yurt Dışı |
| **Firebase Crashlytics**    | Google        | Zorunlu            | Uygulama çökme ve hata raporları                 | Sağlayıcı politikasına göre | Yurt Dışı (Google) |
| **Google Mobile Ads SDK**   | Google        | —                  | Uygulamada yüklü; şu an reklam gösterilmez       | —                       | —                  |
