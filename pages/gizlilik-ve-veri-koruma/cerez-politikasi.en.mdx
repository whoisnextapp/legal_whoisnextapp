...
<Tabs items={["Web (Cookie)", "Mobile app (Tracking/SDK)"]}>
  ...
  <Tab>
    ...
    - **App installation identifier**: the identifier WIN uses for the single-device session and account security (on Android a random identifier generated by the app starting with "wn-"; on iOS the IDFV),
...
In mobile applications, ad-related tracking generally operates via advertising identifiers such as **IDFA/GAID** and is subject to device/OS-level settings.

- **Showing ads:** The Google Mobile Ads (AdMob) SDK is installed in the app, but **no ads are currently shown in the app**. If ads are shown in the future, this policy will be updated and any required permissions (e.g. tracking permission on iOS, explicit consent where required) will be requested separately.
- **Advertising campaign measurement (attribution):** To measure the effectiveness of WIN's own advertising campaigns, the **Meta (Facebook) SDK** and the **TikTok Events SDK** are used in the app; purchase and renewal events may also be forwarded to these platforms via **RevenueCat**. The events and identifiers transmitted are described in Section 4.14 of the [Privacy Policy](/gizlilik-ve-veri-koruma/gizlilik-politikasi). On iOS the advertising identifier (IDFA) is shared only if you grant tracking (ATT) permission.
...
- **Google Mobile Ads / AdMob (Google):** The SDK is installed in the app; **no ads are currently shown**.
- **RevenueCat (USA):** Subscription and purchase validation/management; forwarding device identifiers to Meta and TikTok for advertising measurement.
- **Meta (Facebook) SDK:** App events and advertising campaign measurement (attribution). The Facebook login option has been removed.
- **TikTok Events SDK:** App events and advertising campaign measurement (attribution).
- **Firebase Crashlytics (Google):** App crash and error reports.
...
There is currently no separate "tracking preferences" screen in the app. On iOS you can manage sharing of the advertising identifier through **App Tracking Transparency (ATT)** permission (Settings > Privacy & Security > Tracking). On Android you can reset or delete your advertising ID in the device settings. To withdraw your marketing communication and transfer-abroad consents, you can use the [KVKK Application Form](/basvuru-formlari/kvkk-basvuru-formu) channels.
...
<Tabs items={["Browsers", "iOS", "Android", "Additional opt-out tools"]}>
  ...
  <Tab>
    ...
    - No ads are currently shown in the app; permissions relating to advertising measurement are managed with the device settings described in this section.
...
| **IDFA / GAID (advertising identifiers)** | Apple/Google | Marketing (measurement) | Advertising campaign measurement (attribution) via Meta, TikTok and RevenueCat; on iOS only with ATT permission. No ads are currently shown | Persistent (Resettable) | Abroad |
| **Installation ID (wn-… / IDFV)** | WIN | Mandatory | Single-device session; SMS verification abuse limits | On device: until the app is uninstalled; on server: for the life of the account | Google Cloud / Firebase |
| **RevenueCat SDK**           | RevenueCat    | Mandatory (purchases) | Subscription/purchase validation and management; forwarding device identifiers (IDFV, IDFA if permitted, GAID, Android device ID) and the IP address for attribution | For the duration of the service | Abroad (USA) |
| **Meta (Facebook) SDK**      | Meta          | Marketing (measurement) | App events and advertising attribution      | Per provider policy      | Abroad              |
| **TikTok Events SDK**        | TikTok        | Marketing (measurement) | App events and advertising attribution      | Per provider policy      | Abroad              |
| **Firebase Crashlytics**     | Google        | Mandatory           | App crash and error reports                        | Per provider policy      | Abroad (Google)     |
| **Google Mobile Ads SDK**    | Google        | —                   | Installed in the app; no ads are currently shown   | —                        | —                   |
